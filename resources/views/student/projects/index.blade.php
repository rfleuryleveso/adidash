...
@section('content')
    ...
    <div class="columns is-multiline content-container">
            ...
            @foreach ($group->projects as $project)
                <div class="column is-one-quarter ">
                    <a href="{{ route('student.projects.project.details', ['project' => $project->id]) }}">
                        ...
                        <p class="is-size-7 @if ($project->background_image) has-text-white @else
                                has-text-grey-dark @endif mt-3"><i class="far fa-user"></i>
                                {{ $project->admins->map(function ($user) {
        return $user->fullName;
    })->join(', ') }}
                            </p>
                            @if ($project->ends_at)
                            <p class="is-size-6 @if ($project->background_image) has-text-white @else
                                    has-text-grey-dark @endif"><i class="far fa-calendar"></i>
                                    {{ $project->ends_at->toDateString() }}</p>
                            @endif
                        </div>
                    </a>
                </div>
